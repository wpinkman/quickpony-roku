...
' *********************************************************
' *********************************************************
Function CreateSgScreenSaver() As Object
	this = {
		port: invalid,
		canvas: invalid,
		instareq: invalid,
		...
		InitLayout: SgSaverInitLayout,
		GetData: SgSaverGetData,
		FetchData: SgSaverFetchData,
		WaitForHttp: SgSaverWaitForHttp,
		
		Run: SgSaverRun,
		RunSettings: SgSaverRunSettings,
		ReadRegistry: SgSaverReadRegistry,
		ShowKeyboardScreen: SgSaverShowKeyboardScreen
	}
...
Sub SgSaverRun()
	...
	while true
		...
		GaEvent("SgSaver", "rollover", invalid, count).PostAsync()
	
	end while
		
End Sub

Sub SgSaverInitLayout()
...
Function SgSaverGetData() As Object
...
Sub SgSaverFetchData() As Object
...
Sub SgSaverRunSettings()
		
	LogStartupMessage("SgSaverSettings")
	    ...
	    if type(msg) = "roKeyboardScreenEvent"
	        if msg.isScreenClosed()
	            ...
	            if msg.GetIndex() = 1
		        	...
		        	WriteSgSaver("banner", text)
	                exit while
	            else if msg.GetIndex() = 2
	            	screen.SetText("")
		        	WriteSgSaver("banner", invalid)
	            else
	            	exit while
	            end if
	        endif
...
Sub SgSaverReadRegistry()
'	saverType = ReadSaver("type")
...
'	desc = ReadSaver("description")
'	if desc <> invalid
'		m.description = desc
'	end if
'	
'	m.banner = ReadSaver("banner")

End Sub
					

Function SgSaverWaitForHttp()
